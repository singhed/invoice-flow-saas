...
class AnalyticsService {
  async getDashboardOverview(startDate: Date, endDate: Date): Promise<DashboardOverview> {
    ...
    await redis.setex(cacheKey, config.cache.ttl, JSON.stringify(mockData));
  ...
  async getRevenueTrends(startDate: Date, endDate: Date, period: string): Promise<RevenueTrend[]> {
    const cacheKey = `analytics:trends:${period}:${startDate.toISOString()}:${endDate.toISOString()}`;
    ...
    await redis.setex(cacheKey, config.cache.ttl, JSON.stringify(trends));
  ...
  async getInvoicesByStatus(startDate: Date, endDate: Date): Promise<InvoiceStatusBreakdown[]> {
    const cacheKey = `analytics:status:${startDate.toISOString()}:${endDate.toISOString()}`;
    ...
    await redis.setex(cacheKey, config.cache.ttl, JSON.stringify(breakdown));
  ...
  async getTopCustomers(startDate: Date, endDate: Date, limit: number): Promise<TopCustomer[]> {
    const cacheKey = `analytics:customers:${limit}:${startDate.toISOString()}:${endDate.toISOString()}`;
    ...
    await redis.setex(cacheKey, config.cache.ttl, JSON.stringify(customers));
  ...
  async getPaymentMetrics(startDate: Date, endDate: Date): Promise<PaymentMetrics> {
    const cacheKey = `analytics:payments:${startDate.toISOString()}:${endDate.toISOString()}`;
    ...
    await redis.setex(cacheKey, config.cache.ttl, JSON.stringify(metrics));
  ...
  async trackEvent(event: AnalyticsEvent): Promise<void> {
    ...
    await redis.setex(eventKey, 86400 * 30, JSON.stringify(event));
